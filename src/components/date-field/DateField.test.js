...
const component = <DateField fieldName='test' header='Test Date Field' hint='Enter test date' values={{ day: 9, month: 5, year: 2022 }}/>;

describe('Should render the GDS date field', () => {
  it('should have correct fields', () => {
    render(component);

    expect(screen.getByRole('heading', {name: 'Test Date Field'})).toBeDefined();
  ...
  it('should style elements according to GDS', () => {
    ...
    expect(heading.closest("legend")).toHaveClass('govuk-fieldset__legend govuk-fieldset__legend--l');
    ...
    fields.forEach((field) => {
      ...
      expect(field.previousSibling).toHaveClass('govuk-label govuk-date-input__label');
      const formGroup = field.closest('div');
      expect(formGroup).toHaveClass('govuk-form-group');
      expect(formGroup.closest('#test-date-range-fields > div')).toHaveClass('govuk-date-input__item');
    })
  });
