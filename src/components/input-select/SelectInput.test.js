...
const component = (
  <SelectInput label="test select" options={testOptions} onChange={mockCallback} initialValue="testTwo" />
);
...
describe('Handle user input', () => {
  ...
  it('Should allow keyboard users to update options', () => {
    ...
    fireEvent.keyDown(comboboxInput, { key: 'Enter', keyCode: 13, charCode: 13 });
    expect(comboboxInput).toHaveAttribute('aria-expanded', 'true');
    fireEvent.keyDown(comboboxInput, { key: 'ArrowUp', keyCode: 38, charCode: 38 });
    expect(options[0]).toHaveClass('option-current');
    fireEvent.keyDown(comboboxInput, { key: 'Enter', keyCode: 13, charCode: 13 });
